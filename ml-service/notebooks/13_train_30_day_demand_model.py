"""Train a leakage-safe 30-day demand model for large stores with stockrooms."""
...
METRICS_PATH = (
    Path(__file__).resolve().parents[1]
    / "models"
    / "demand_model_30_day_metrics.csv"
)
ERRORS_PATH = (
    Path(__file__).resolve().parents[1]
    / "models"
    / "demand_model_30_day_prediction_errors.csv"
)
...
def analyze_prediction_errors(
    ...
    errors["ThirtyDayBaselinePrediction"] = baseline_predictions
...
def save_outputs(
    ...
    metrics_rows = [
        {
            "model": "store_product_30_day_baseline",
...
def main() -> None:
    ...
    print("30-day large-store demand model")
    ...
    baseline_metrics = evaluate(
        "Store-product 30-day baseline",
        test[TARGET],
        baseline_predictions,
    )

    pipeline = build_model_pipeline()
    print("\nTraining 30-day large-store demand model...")
    ...
    model_metrics = evaluate(
        "Hist Gradient Boosting 30-day model",
        test[TARGET],
        model_predictions,
    )
    improvement = (
