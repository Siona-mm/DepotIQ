...
const FIELDS = [
  ["name", "Name", "text"], ["category", "Category", "text"], ["brand", "Brand", "text"],
  ["supplierCode", "Supplier code", "text"], ["externalSku", "External SKU / barcode", "text"], ["unitCost", "Unit cost", "number"],
...
export default function ProductsView({ collapsed, onAction, onCollapse, onNavigate, onSignOut, permissions, profile, user }) {
  ...
  const save = async (event) => { event.preventDefault(); setSaving(true); setError(""); try { const payload = { ...form, unitCost: Number(form.unitCost), price: Number(form.price), weightKg: Number(form.weightKg), shelfLifeDays: Number(form.shelfLifeDays) }; const editableFields = { ...payload }; delete editableFields.productCode; const saved = editing ? await updateProduct(editing.id, editableFields) : await createProduct(editableFields); setProducts((current) => editing ? current.map((product) => product.id === saved.id ? saved : product) : [...current, saved]); setFormOpen(false); } catch (requestError) { setError(requestError.message); } finally { setSaving(false); } };
