import { Camera, LogOut, Save, Settings, ShieldCheck, UserRound } from "lucide-react";
import { useRef, useState } from "react";
import { updateProfile } from "../api/depotiqApi.js";
...
export default function ProfileView({ collapsed, onAction, onCollapse, onNavigate, onProfileUpdated, onSignOut, profile: initialProfile, user }) {
  ...
  const save = async (event) => {
    ...
    try { const saved = await updateProfile(profile); setProfile(saved); onProfileUpdated(saved); setMessage("Profile saved."); }
    catch (requestError) { setError(requestError.message); }
    finally { setSaving(false); }
  };

  return <div className={collapsed ? "app-shell sidebar-collapsed" : "app-shell"}>
    <AppSidebar activePage="Profile" collapsed={collapsed} onAction={onAction} onCollapse={onCollapse} onNavigate={onNavigate} onSignOut={onSignOut} profile={profile} user={user} />
    <main className="dashboard profile-page">
      <header className="topbar"><h1>Profile</h1><div /><UserAvatar profile={profile} user={user} /></header>
      ...
      <section className="profile-layout">
        ...
        <div className="profile-content"><form className="profile-form" onSubmit={save}><header><UserRound size={18} /><div><h2>Profile details</h2><p>These details are visible across your DepotIQ workspace.</p></div></header><div className="profile-fields"><label>Display name<input disabled={loading} onChange={(event) => updateField("displayName", event.target.value)} required value={profile.displayName ?? ""} /></label><label>Email address<input disabled={loading} onChange={(event) => updateField("email", event.target.value)} type="email" value={profile.email ?? ""} /></label><label>Job title<input disabled={loading} onChange={(event) => updateField("jobTitle", event.target.value)} value={profile.jobTitle ?? ""} /></label><label>Username<input disabled value={user.username} /></label></div><footer><button className="primary-button" disabled={loading || saving} type="submit"><Save size={16} />{saving ? "Saving..." : "Save profile"}</button></footer></form><section className="profile-account-card"><header><ShieldCheck size={18} /><div><h2>Account and workspace</h2><p>Manage your access and operating preferences.</p></div></header><div className="profile-account-row"><span>Access level</span><strong>{user.roles?.[0]?.replace("ROLE_", "")?.toLowerCase() || "user"}</strong></div><div className="profile-account-actions"><button className="secondary-button" onClick={() => onNavigate("Settings")} type="button"><Settings size={15} />Workspace settings</button><button className="secondary-button danger-text" onClick={onSignOut} type="button"><LogOut size={15} />Sign out</button></div></section></div>
      </section>
    </main>
  </div>;
}
