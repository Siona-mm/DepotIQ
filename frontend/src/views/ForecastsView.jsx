...
export default function ForecastsView({ collapsed, onAction, onCollapse, onNavigate, onSignOut, permissions, user }) {
  ...
  return <div className={collapsed ? "app-shell sidebar-collapsed" : "app-shell"}>
    ...
    <main className="dashboard forecasts-page">
      <header className="topbar"><h1>Forecasts</h1><label className="search-box"><Search size={15} /><span className="sr-only">Search forecasts</span><input onChange={(event) => setQuery(event.target.value)} placeholder="Search store, product, category, or model..." value={query} /></label><ProfileMenu onSignOut={onSignOut} user={user} /></header>
