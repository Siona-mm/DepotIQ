import { Boxes, CircleAlert, Search, Store } from "lucide-react";
import { useCallback, useEffect, useMemo, useState } from "react";
import { loadStoreInventory } from "../api/depotiqApi.js";
import AppSidebar from "../components/AppSidebar.jsx";

const number = (value) => new Intl.NumberFormat("en-US").format(Number(value ?? 0));
const state = (item) => Number(item.inventoryLevel ?? 0) === 0 ? "OUT" : Number(item.inventoryLevel ?? 0) < 25 ? "LOW" : "HEALTHY";

export default function StoreInventoryView({ collapsed, onAction, onCollapse, onNavigate }) {
  ...
  return <div className={collapsed ? "app-shell sidebar-collapsed" : "app-shell"}><AppSidebar activePage="Store Inventory" collapsed={collapsed} onAction={onAction} onCollapse={onCollapse} onNavigate={onNavigate} />
    <main className="dashboard store-inventory-page"><header className="topbar"><h1>Store Inventory</h1><label className="search-box"><Search size={15} /><span className="sr-only">Search store inventory</span><input onChange={(event) => setQuery(event.target.value)} placeholder="Search store, product, or category..." value={query} /></label></header>
      ...
      <section className="table-panel"><div className="panel-toolbar"><h2>Store stock levels</h2><span>{visible.length} shown</span></div><div className="table-scroll"><table><thead><tr><th>Store</th><th>Product</th><th>Category</th><th>On hand</th><th>Incoming</th><th>Status</th><th>Updated</th></tr></thead><tbody>{visible.map((item) => <tr key={item.id}><td>{item.storeCode}</td><td>{item.productCode}</td><td>{item.category}</td><td>{number(item.inventoryLevel)}</td><td>{number(item.incomingUnits)}</td><td><span className={`priority ${state(item).toLowerCase()}`}>{state(item)}</span></td><td>{item.lastUpdated ? new Date(item.lastUpdated).toLocaleDateString() : "—"}</td></tr>)}</tbody></table></div>{loading && <div className="panel-empty">Loading store inventory...</div>}{!loading && !visible.length && <div className="panel-empty">No inventory records match your search.</div>}</section>
    </main></div>;
}
